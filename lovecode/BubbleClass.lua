...
BubbleClass.addMatch = function(bubble, matchedBubble)
	assert (bubble.id ~= matchedBubble.id, "addMatch : bubble "..bubble.id.." tried to add itself")
	assert (bubble:sameColorAs(matchedBubble), "addMatch : "..bubble.id.." tried to add "..matchedBubble.id.." which did not match")
...
BubbleClass.checkMatch = function(bubble)
-- use sparingly
	for n, match in ipairs(bubble.colorMatchDict) do
		if (not bubble:sameColorAs(match)) then
			table.remove(bubble.colorMatchDict, n)
			bubble.colorMatchCount = bubble.colorMatchCount - 1
		end
	end
...
BubbleClass.newRoot = function(x,c)
	assert (BubbleClass.acceptedPosition(x, bubbleRadius, bubbles), "There is already a bubble here! ".."("..x.." , "..bubbleRadius)
...
bubbles.initialize = function()
	...
	-- break generated match
	foundMatch = true
	while(foundMatch) do
		foundMatch = false
		for _, b in ipairs(bubbles) do
			if (b.colorMatchCount >= bubbleSequenceSize) then
				foundMatch = true
				b.color = getRandomColor()
				for _,b2 in ipairs(bubbles) do
					b2:checkMatch()
				end
			end
		end
